...
export function getPracticeProofSecret(): string {
  ...
  const sitemap = process.env.SITEMAP_SECRET?.trim();
  if (sitemap) return sitemap;

  throw new Error(
    '[practice] PRACTICE_SUBMIT_SECRET is required in production when using signed practice RPCs without a service-role key.',
  );
}
