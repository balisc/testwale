...
import { logPracticeDebug } from '@/lib/practiceDebugLog';
import {
  buildPhaseLabel,
...
const COPY = {
  en: {
    ...
    errorStaleQuestions: 'Questions were updated. Your practice session has been refreshed.',
  ...
  hi: {
    ...
    errorStaleQuestions: 'प्रश्न अपडेट हो गए हैं। आपका अभ्यास सत्र रीफ्रेश हो गया है।',
...
export default function QuestionPractice({
  ...
  const resolvedSubjectSlug = subjectSlug?.trim() || null;
  const resolvedTopicSlug =
    topicSlug?.trim() ||
    (typeof seoTopic === 'string' ? seoTopic.trim() : null) ||
    null;
  const resolvedSubtopicSlug =
    subtopicSlug?.trim() ||
    (typeof seoSubtopic === 'string' ? seoSubtopic.trim() : null) ||
    null;
  ...
  const validatedSessionIdsRef = useRef<Set<string>>(new Set());
  ...
  // Replace SSR/Data-Cache UUIDs if any no longer exist in public.questions.
  useEffect(() => {
    if (!isSubtopicBatchMode || !questionBatchScopeId) return;
    ...
    void (async () => {
      ...
      if (candidateIds.length === 0) {
        logPracticeDebug('[practice/session] validate SSR/batch IDs', {
          status: 'empty',
          requestedCount: 0,
          validCount: 0,
          missingCount: 0,
      ...
      try {
        const res = await fetch('/api/practice/validate-question-ids', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ questionIds: candidateIds }),
          cache: 'no-store',
        });
        ...
        const validIds = Array.isArray(payload.validIds) ? payload.validIds : [];
        const missingIds = Array.isArray(payload.missingIds)
          ? payload.missingIds
          : Array.isArray(payload.staleQuestionIds)
            ? payload.staleQuestionIds
            : [];

        logPracticeDebug('[practice/session] validate SSR/batch IDs', {
          status: !res.ok ? 'http_error' : (payload.status ?? (missingIds.length ? 'stale' : 'ok')),
          requestedCount: payload.requestedCount ?? candidateIds.length,
          validCount: payload.validCount ?? validIds.length,
          missingCount: payload.missingCount ?? missingIds.length,
          ...
          supabaseError: payload.supabaseError
            ? {
                code: payload.supabaseError.code ?? null,
                message: payload.supabaseError.message ?? null,
                details: payload.supabaseError.details ?? null,
                hint: payload.supabaseError.hint ?? null,
              }
            : null,
        });
        ...
        validatedSessionIdsRef.current = new Set(validIds);

        if (missingIds.length === 0 && (payload.allLive || validIds.length === candidateIds.length)) {
          return;
        }

        if (cancelled || requestGenerationRef.current !== generationAtStart) return;

        // Strip deleted IDs from local state immediately — do not wait for refetch.
        const validSet = new Set(validIds);
        setLoadedQuestions((prev) => prev.filter((question) => validSet.has(question.id)));
        setCurrentPageIds((prev) => prev.filter((id) => validSet.has(id)));
        setResultsByQuestion((prev) => {
          const next: Record<string, QuestionResult> = {};
          for (const [id, value] of Object.entries(prev)) {
            if (validSet.has(id)) next[id] = value;
          }
          return next;
        });
        setAttemptedIds((prev) => new Set([...prev].filter((id) => validSet.has(id))));

        setSubmitError(c.errorStaleQuestions);
        const generation = ++requestGenerationRef.current;
        activeScopeKeyRef.current = scopeKey;
        await restartPublicScan(generation, scopeKey);
        validatedSessionIdsRef.current = new Set(
          loadedQuestionsRef.current.map((question) => question.id),
        );
        setIndex(0);
        setSelectedOption(null);
        setSubmitted(false);
      } catch (error) {
        logPracticeDebug('[practice/session] validate SSR/batch IDs failed', {
          status: 'error',
          requestedCount: candidateIds.length,
          validCount: 0,
          missingCount: candidateIds.length,
          requestedIds: candidateIds,
          validIds: [],
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- intentional scope-gated refresh
  }, [isSubtopicBatchMode, questionBatchScopeId, practiceScopeKey]);

  // Keep validated session ID set in sync with the displayed batch (prevents hydration reintroducing stale IDs).
  useEffect(() => {
    if (!isSubtopicBatchMode) return;
    validatedSessionIdsRef.current = new Set(loadedQuestions.map((question) => question.id));
  }, [isSubtopicBatchMode, loadedQuestions]);
  ...
  const loadNextBatch = useCallback(
    async (options?: { manual?: boolean; cursor?: string | null }): Promise<boolean> => {
      ...
      if (!isSubtopicBatchMode || !cursorToFetch || batchFetchInFlightRef.current) return false;
      if (!options?.manual && fetchedCursorsRef.current.has(cursorToFetch)) return false;
      ...
      try {
        ...
        batchFetchInFlightRef.current = false;
        if (!isStaleGeneration(generation, scopeKey) && !batchCompleteInFlightRef.current) {
  ...
  const advanceToNextQuestionPage = useCallback(
    async (completedQuestionIds: string[]) => {
      if (!isSubtopicBatchMode) return;
      if (batchCompleteInFlightRef.current || batchFetchInFlightRef.current) return;

      const fallbackCursor =
        completedQuestionIds.length > 0
          ? completedQuestionIds[completedQuestionIds.length - 1]
          : null;
      ...
      const canFetchMore = Boolean(cursor && likelyHasMore);
      ...
      if (!canFetchMore && !canAdvanceCycle) return;

      batchCompleteInFlightRef.current = true;
      setLoadingMoreBatches(true);
      setBatchLoadError(null);

      if (isSubtopicMasteryMode && completedQuestionIds.length > 0) {
        setSessionPassRemovedIds((prev) => {
          const next = new Set(prev);
          for (const id of completedQuestionIds) next.add(id);
          return next;
        });
        setEligibleQuestionIds((prev) => {
          const next = new Set(prev);
          for (const id of completedQuestionIds) next.delete(id);
          return next;
        });
      }
      ...
      try {
        ...
        if (canFetchMore && cursor) {
          loadedNew = await loadNextBatch({ manual: true, cursor });
        }
  ...
  const maybeAdvanceMasteryBatch = useCallback(() => {
    ...
    if (batchCompleteInFlightRef.current) return;
  ...
  const submitAnswer = useCallback(async () => {
    ...
    const displayedQuestionId = String(current.id ?? '').trim();
    const requestStartedAt = new Date().toISOString();
    const attemptNumber = resultsByQuestion[displayedQuestionId] ? 2 : 1;
    const sessionId = practiceScopeKey;
    const inCurrentState =
      loadedQuestionsRef.current.some((q) => q.id === displayedQuestionId) ||
      questions.some((q) => q.id === displayedQuestionId) ||
      validatedSessionIdsRef.current.has(displayedQuestionId) ||
      (!isSubtopicBatchMode && Boolean(current.id));

    const baseLog = {
      questionId: displayedQuestionId || null,
      selectedOption,
      attemptNumber,
      subjectSlug: resolvedSubjectSlug,
      topicSlug: resolvedTopicSlug,
      subtopicSlug: resolvedSubtopicSlug,
      sessionId,
      questionExistsInCurrentState: inCurrentState,
      requestStartedAt,
    ...
    if (!PRACTICE_UUID_RE.test(displayedQuestionId) || !['A', 'B', 'C', 'D'].includes(selectedOption)) {
      logPracticeDebug('[practice/submit] client attempt', {
        ...baseLog,
        responseStatus: 400,
        responseCode: 'INVALID_REQUEST',
        responseMessage: 'Client rejected invalid questionId/selectedOption',
      });
      setSubmitError(c.errorSubmit);
      return;
    }

    if (isSubtopicBatchMode && !inCurrentState) {
      logPracticeDebug('[practice/submit] client attempt', {
        ...baseLog,
        responseStatus: 409,
        responseCode: 'STALE_QUESTION',
        responseMessage: 'Question missing from validated session — refreshing without API call',
      });
      setSubmitError(c.errorStaleQuestions);
      setSelectedOption(null);
      setSubmitted(false);
      const generation = ++requestGenerationRef.current;
      activeScopeKeyRef.current = practiceScopeKey;
      await restartPublicScan(generation, practiceScopeKey);
      validatedSessionIdsRef.current = new Set(
        loadedQuestionsRef.current.map((question) => question.id),
      );
      setIndex(0);
    ...
    try {
      logPracticeDebug('[practice/submit] client attempt', baseLog);
      ...
      const response = await fetch('/api/practice/submit', {
        ...
        body: JSON.stringify({
          ...
          selectedOption,
          timeTakenSeconds,
          subtopicId: subtopicId ?? questionBatchScopeId ?? null,
          topicId: topicId ?? null,
          subjectSlug: resolvedSubjectSlug,
          topicSlug: resolvedTopicSlug,
          subtopicSlug: resolvedSubtopicSlug,
          cacheSource: isSubtopicBatchMode ? 'loadedQuestions' : 'questions-prop',
        }),
        cache: 'no-store',
      });

      const payload = (await response.json()) as QuestionResult & {
        ok?: boolean;
        error?: string;
        code?: string;
        message?: string;
      ...
      const responseCode = payload.code ?? payload.error ?? null;
      const responseMessage = payload.message ?? payload.details?.message ?? null;
      ...
      const isStaleQuestion =
        response.status === 409 &&
        (responseCode === 'STALE_QUESTION' ||
          payload.error === 'STALE_QUESTION' ||
          payload.refreshSession === true);
      ...
      if (!response.ok) {
        logPracticeDebug('[practice/submit] client details', {
          responseStatus: response.status,
          responseCode,
          responseMessage,
          questionId: displayedQuestionId,
        ...
        setSubmitError(
          process.env.NODE_ENV !== 'production' && responseCode
            ? `${c.errorSubmit} [${responseCode}${responseMessage ? ` | ${responseMessage}` : ''}]`
            : c.errorSubmit,
        );
        return;
      }
      ...
      logPracticeDebug('[practice/submit] client attempt', {
        ...
        topicSlug: resolvedTopicSlug,
        subtopicSlug: resolvedSubtopicSlug,
        sessionId: practiceScopeKey,
        questionExistsInCurrentState: true,
        requestStartedAt: null,
        responseStatus: null,
        responseCode: 'CLIENT_EXCEPTION',
