-- QuestionWale: user profile page (run once in Supabase SQL Editor)
-- Requires: public.users, public.user_question_attempts, catalog tables

-- ===========================================================================
-- user_profiles (1 row per user — editable profile fields)
-- ===========================================================================
...
create table if not exists public.user_profiles (
  ...
  daily_goal integer not null default 50 check (daily_goal between 1 and 500),
  weekly_goal integer not null default 300 check (weekly_goal between 1 and 5000),
  monthly_goal integer not null default 1500 check (monthly_goal between 1 and 20000),
...
-- API uses service role / security definer RPC after cookie auth
drop policy if exists "user_profiles_no_direct" on public.user_profiles;
create policy "user_profiles_no_direct"
  on public.user_profiles for all to anon, authenticated
  using (false) with check (false);

-- ===========================================================================
-- bookmarks & notes (counts for profile widgets)
-- ===========================================================================
...
create table if not exists public.user_bookmarks (
  ...
  constraint user_bookmarks_unique unique (user_id, question_id)
);
...
drop policy if exists "user_bookmarks_no_direct" on public.user_bookmarks;
create policy "user_bookmarks_no_direct" on public.user_bookmarks for all to anon, authenticated using (false) with check (false);

drop policy if exists "user_notes_no_direct" on public.user_notes;
create policy "user_notes_no_direct" on public.user_notes for all to anon, authenticated using (false) with check (false);

-- ===========================================================================
-- Helper: readiness label from score
-- ===========================================================================

create or replace function public.readiness_label(p_score numeric)
returns text
language sql
immutable
as $$
  select case
    ...
    when p_score >= 45 then 'Average'
    else 'Needs Work'
  end;
$$;

-- ===========================================================================
-- RPC: full profile page payload for one user
-- ===========================================================================
